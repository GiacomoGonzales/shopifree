...
import { CategoryWithId, CategoryName, CategoryDescription } from '../../lib/categories'
...
export default function CategoryModal({
    ...
    if (isOpen) {
      if (category) {
        ...
        setFormData({
          ...
          description: category.description[locale as keyof typeof category.description] || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const categoryData = {
        name: {
          es: locale === 'es' ? formData.name.trim() : (category?.name.es || formData.name.trim()),
          en: locale === 'en' ? formData.name.trim() : (category?.name.en || formData.name.trim())
        } as CategoryName,
        description: {
          es: locale === 'es' ? formData.description.trim() : (category?.description.es || formData.description.trim()),
          en: locale === 'en' ? formData.description.trim() : (category?.description.en || formData.description.trim())
                ...
                {parentCategories
                  ...
                  .map((cat) => (
                    <option key={cat.id} value={cat.id}>
                      {cat.name[locale as keyof typeof cat.name] || cat.name.es}
                    </option>
                  ))}
              </select>
