    ...
    <div className="min-h-screen bg-white">
      ...
      >
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            {store.logoUrl ? (
              <img src={store.logoUrl} alt={store.storeName} className="h-8 w-auto" />
            ) : (
          ...
          >
            ...
            {totalItems > 0 && (
              <span className="absolute -top-0.5 -right-0.5 w-5 h-5 bg-gray-900 text-white text-[10px] font-medium rounded-full flex items-center justify-center">
                {totalItems}
              </span>
            )}
          </button>
      ...
      <section className="pt-28 pb-12 px-5">
        <div className="max-w-6xl mx-auto text-center">
          ...
          {store.slogan && (
            <p className="text-gray-500 text-lg">{store.slogan}</p>
          )}
        </div>
      </section>
              ...
              >
                <div className="relative aspect-square mb-4 rounded-2xl overflow-hidden bg-gray-50">
                  ...
                  <button
                    ...
                    className="absolute bottom-3 right-3 w-10 h-10 bg-white rounded-full shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-200 hover:scale-110 active:scale-95"
                  >
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              {store.whatsappNumber && (
                <p className="text-sm text-gray-500 mt-1">WhatsApp: {store.whatsappNumber}</p>
              )}
            </div>
            <a
      ...
      {store.whatsappNumber && totalItems === 0 && (
        <a
          href={`https://wa.me/${store.whatsappNumber.replace(/\D/g, '')}?text=${encodeURIComponent(`Hola! Vi tu catálogo ${store.storeName}`)}`}
          target="_blank"
          rel="noopener noreferrer"
          className="fixed bottom-6 right-6 w-14 h-14 bg-[#25D366] rounded-full shadow-lg flex items-center justify-center hover:scale-110 active:scale-95 transition-transform z-50"
        >
      ...
      {/* Bottom Cart Bar */}
      {totalItems > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 px-5 py-4 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
          <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="font-semibold text-gray-900">{formatPrice(totalPrice, store.currency)}</p>
              </div>
            </button>

            <button
              onClick={sendWhatsAppOrder}
              className="flex items-center gap-2 bg-[#25D366] text-white px-6 py-3 rounded-full font-medium hover:bg-[#20BD5A] active:scale-95 transition-all"
            >
      ...
      {selectedProduct && (
        ...
        >
          ...
          <div
            className="relative w-full max-w-lg max-h-[90vh] bg-white rounded-t-3xl md:rounded-3xl overflow-hidden animate-slideUp"
          ...
          >
            ...
            <div className="p-6">
              ...
              <button
                ...
                className="w-full py-4 bg-gray-900 text-white rounded-full font-medium hover:bg-gray-800 active:scale-[0.98] transition-all"
              >
                Agregar al pedido
              </button>
            </div>
            ...
            {items.length > 0 && (
              <div className="absolute bottom-0 left-0 right-0 p-5 border-t border-gray-100 bg-white">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-xl font-semibold text-gray-900">{formatPrice(totalPrice, store.currency)}</span>
                </div>
                <button
                  onClick={() => {
                    setIsCartOpen(false);
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-[#25D366] text-white py-4 rounded-full font-medium hover:bg-[#20BD5A] active:scale-[0.98] transition-all"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                  </svg>
