...
const FavoritesClientPage = () => {
  ...
  const ThemeFavorites = useMemo(() => {
    ...
    return dynamic<{ tienda: Tienda }>(
      () => import(`../../../themes/${store.theme}/Favoritos`).then(mod => mod.default).catch(() => {
        console.log(`Theme Favorites ${store.theme} not found, using default`)
        return null
      }),
      { 
        ssr: false,
  ...
  const FavoritesContent = () => {
    // Si hay un componente específico del tema, usarlo
    if (ThemeFavorites) {
      return <ThemeFavorites tienda={convertStoreToTienda(store)} />
    }
    
    // Si no, usar el componente por defecto
